...
const WorldMapPlaceholder = ({
  ...
  const getBulletProps = (count: number) => {
    if (count === 0) return { r: 4, color: "hsl(210, 60%, 60%)", glow: false };
    const intensity = count / maxCount;
    const isPeak = count > avgCount;
    let color = "hsl(210, 80%, 55%)"; // blue
    if (intensity > 0.33) color = "hsl(45, 100%, 50%)"; // yellow
    if (intensity > 0.66) color = "hsl(0, 80%, 55%)"; // red
    const r = isPeak ? 6 + intensity * 4 : 4 + intensity * 2;
  ...
  return (
    <div className="w-full h-full relative">
            ...
            >
              ...
              {glow && (
                <>
                  ...
                  <circle cx={pt.cx} cy={pt.cy} r={r} fill="none" stroke={color} strokeWidth={0.8} opacity={0.2}>
                    <animate attributeName="r" values={`${r};${r + 8}`} dur="2s" begin="0.5s" repeatCount="indefinite" />
              ...
              {isSelected && (
                <circle cx={pt.cx} cy={pt.cy} r={r + 4} fill="none" stroke="hsl(var(--primary))" strokeWidth={2}>
                  <animate attributeName="r" values={`${r + 4};${r + 14}`} dur="1.5s" repeatCount="indefinite" />
              ...
              {/* Inner bright dot */}
              <circle cx={pt.cx} cy={pt.cy} r={Math.max(r * 0.35, 1.5)} fill="white" opacity={0.7} className="pointer-events-none" />
      ...
      {activeTrend && (
        <div
          className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-card border border-border rounded-2xl p-4 shadow-lg max-w-sm w-[90%] animate-in fade-in slide-in-from-bottom-4 duration-300"
        ...
        >
          ...
          <p className="text-[10px] text-muted-foreground mt-2">Clique para fechar</p>
        </div>
      )}
    </div>
  );
