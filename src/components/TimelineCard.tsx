...
const SOURCE_BADGES: Record<string, { label: Record<string, string>; icon: string; css: string; explanation: Record<string, string> }> = {
  imprensa: { label: { pt: "Imprensa", en: "Press" }, icon: "✓", css: "source-badge-press", explanation: { pt: "Veículos de imprensa profissional com equipe editorial", en: "Professional press outlets with editorial teams" } },
  dados_oficiais: { label: { pt: "Oficial", en: "Official" }, icon: "◆", css: "source-badge-official", explanation: { pt: "Instituições governamentais e organismos internacionais", en: "Government institutions and international organizations" } },
  cientifico: { label: { pt: "Acadêmico", en: "Academic" }, icon: "◈", css: "source-badge-academic", explanation: { pt: "Publicações científicas revisadas por pares", en: "Peer-reviewed scientific publications" } },
  enciclopedico: { label: { pt: "Enciclopédico", en: "Encyclopedic" }, icon: "◎", css: "source-badge-encyclopedic", explanation: { pt: "Plataformas de conhecimento colaborativo", en: "Collaborative knowledge platforms" } },
  redes_sociais: { label: { pt: "Social", en: "Social" }, icon: "◉", css: "source-badge-social", explanation: { pt: "Redes sociais e comunidades online", en: "Social networks and online communities" } },
  google_trends: { label: { pt: "Buscas", en: "Searches" }, icon: "◉", css: "source-badge-search", explanation: { pt: "Dados de volume de buscas em mecanismos de pesquisa", en: "Search engine volume data" } },
};

/* ─── Term explanations ─── */
const TERM_EXPLANATIONS: Record<string, Record<string, string>> = {
  pt: {
    "CPIAUCSL": "CPI = Índice de Preços ao Consumidor dos EUA",
    "PMID": "PMID = identificador de artigo no PubMed",
    "FRED": "FRED = base de dados econômicos do Fed de St. Louis",
    "DOI": "DOI = identificador digital de publicação acadêmica",
    "GDP": "GDP = Produto Interno Bruto",
    "CPI": "CPI = Índice de Preços ao Consumidor",
    ...
    "NFP": "NFP = Folha de Pagamento Não-Agrícola dos EUA",
    "ARXIV": "arXiv = repositório de preprints científicos",
  },
  en: {
    "CPIAUCSL": "CPI = US Consumer Price Index",
    "PMID": "PMID = PubMed article identifier",
    ...
    "IBGE": "IBGE = Brazilian Institute of Geography and Statistics",
...
const TimelineCard = ({
  ...
  const tags = useMemo(() => {
    ...
    if (trustBadge === "verified" || trustBadge === "press") {
      ...
      result.push({ label: "◆ " + (lang === "pt" ? "Oficial" : "Official"), css: "source-badge-official" });
    }
  ...
  return (
    ...
    >
      ...
      {/* ② Title + thumbnail */}
      <div className={`flex gap-2 mb-1 ${compact ? "" : ""}`}>
        <div className="flex-1 min-w-0">
          <h3 className={`font-semibold text-foreground leading-snug ${compact ? "text-[11px] line-clamp-1" : "text-[12px] line-clamp-2"}`}
      ...
      <div className="flex items-end gap-2 mt-1">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {sources && sources.length > 1 && (
            <Tooltip>
              <TooltipTrigger asChild>
                <span className="text-[8px] text-muted-foreground tabular-nums cursor-help">
                  {sources.length}src
                </span>
              </TooltipTrigger>
              <TooltipContent side="top" className="text-[10px]">
                {lang === "pt" ? `${sources.length} fontes independentes confirmam` : `${sources.length} independent sources confirm`}
              </TooltipContent>
            </Tooltip>
          )}
        </div>
      ...
      {propagationSources && !compact && (
        <div className="flex items-center gap-1 mt-1.5 pt-1.5 border-t border-border/15 overflow-hidden">
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="text-[7px] text-muted-foreground/40 uppercase tracking-wider flex-shrink-0 cursor-help">
                PROP
              </span>
            </TooltipTrigger>
            <TooltipContent side="top" className="text-[10px] max-w-[200px]">
              {lang === "pt" ? "Propagação: caminho entre fontes que cobrem esta tendência" : "Propagation: path between sources covering this trend"}
            </TooltipContent>
          </Tooltip>
