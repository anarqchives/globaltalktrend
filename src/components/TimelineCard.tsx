      ...
      <div className={`timeline-card group ${expanded ? 'timeline-card-expanded' : ''}`} data-tier={trendScore >= 70 ? "critical" : trendScore >= 40 ? "moderate" : "low"}>
        ...
        {expanded && (
          <div className="p-3 max-h-[500px] overflow-y-auto">
            ...
            {/* ── METRICS GRID 4×1 compact ── */}
            <div className="grid grid-cols-4 gap-1 mb-3">
              {[
                { value: String(trendScore), label: lang === "pt" ? "TVI" : "TVI", highlight: trendScore >= 60 },
                { value: change, label: lang === "pt" ? "Crescimento" : "Growth" },
                { value: volume || "—", label: "Volume" },
                { value: String(sources?.length || 1), label: lang === "pt" ? "Fontes" : "Sources" },
              ].map((m, i) => (
                <div key={i} className="text-center rounded-md px-2 py-1.5 bg-secondary/50">
                  <span className={`block text-sm font-bold leading-none ${m.highlight ? 'text-destructive' : changePositive && i === 1 ? 'text-emerald-600 dark:text-emerald-400' : 'text-foreground'}`}>
                    {m.value}
                  </span>
                  <span className="block text-[9px] text-muted-foreground mt-0.5 uppercase tracking-wide">{m.label}</span>
                </div>
              ))}
            </div>
