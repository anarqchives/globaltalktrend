...
import {
  ...
  Clock, BarChart3, Zap, ChevronDown, ChevronUp, Bell, Share2 } from
"lucide-react";
...
function CriticalSquare({ m, i, lang, t, isExpanded, onToggle, onSelectTrend
...
}: {m: CriticalMoment;i: number;lang: LangCode;t: (k: any) => string;isExpanded: boolean;onToggle: () => void;onSelectTrend?: (trend: any) => void;}) {
  const trend = m.trend;
  ...
  if (m.changePercent > 500) explParts.push(`crescimento explosivo de ${Math.round(m.changePercent)}%`);else
  if (m.changePercent > 200) explParts.push(`alta aceleração de ${Math.round(m.changePercent)}%`);else
  if (m.changePercent > 50) explParts.push(`crescimento de ${Math.round(m.changePercent)}%`);
    ...
    <motion.div
      ...
      <div className="p-4">
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          <h3 className="text-sm text-foreground leading-snug line-clamp-2 flex-1 font-medium">
            {trend.title}
          </h3>
